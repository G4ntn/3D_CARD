...
function Band({ maxSpeed = 50, minSpeed = 10 }) {
  ...
  useEffect(() => {
    if (card.current) {
      console.log('Card position:', card.current.translation());
    }
    if (materials.base) {
      console.log('Base material map:', materials.base.map);
    }
  }, [materials]);
    ...
    <>
      <group position={[2, 4, 0]}>
        ...
        <RigidBody position={[2, 0, 0]} ref={card} {...segmentProps} type={dragged ? 'kinematicPosition' : 'dynamic'}>
          ...
          >
            {/* Changed material to hotpink for visibility test */}
  ...
  // --- Optional: Render without physics (uncomment to test) ---
  /*
  return (
    <group position={[2, 4, 0]}>
