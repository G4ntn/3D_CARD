...
export function Band({ maxSpeed = 50, minSpeed = 10 }) {
  ...
  // Log the nodes to see what they're named
  useEffect(() => {
    ...
    <>
      <group position={[2, 4, 0]}>
        ...
        >
          ...
          >
            {/* TEMP fallback: just render whole scene */}
            {/* Replace with correct nodes when known */}
            <primitive object={nodes?.card || nodes?.Card || nodes?.Scene} />
          </group>
        </RigidBody>
      </group>
      <mesh ref={band}>
